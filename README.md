...
| File | What changed |
|---|---|
| `vllm/sampling_params.py` | `prompt_confidence_only`, `confidence_start_idx` fields + auto-config |
| `vllm/outputs.py` | `mean_prompt_confidence` on `RequestOutput` |
| `vllm/config/model.py` | `confidence_micro_chunk` field on `ModelConfig` |
| `vllm/engine/arg_utils.py` | `--confidence-micro-chunk` CLI arg |
| `vllm/entrypoints/openai/completion/protocol.py` | Request/response fields |
| `vllm/entrypoints/openai/completion/serving.py` | Validation, per-prompt override, response wiring |
| `vllm/v1/outputs.py` | `mean_prompt_confidence_dict` on `ModelRunnerOutput` |
| `vllm/v1/engine/__init__.py` | `mean_prompt_confidence` on `EngineCoreOutput` |
| `vllm/v1/engine/logprobs.py` | `mean_prompt_confidence` on `LogprobsProcessor` |
| `vllm/v1/engine/output_processor.py` | Extract + pass to `RequestOutput` |
| `vllm/v1/core/sched/scheduler.py` | Extract from runner output, pass to engine output |
| `vllm/v1/worker/gpu_model_runner.py` | Core GPU computation (micro-chunk path, accumulation, finalization) |
